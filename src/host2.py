...
class Level5:
	def send(self, string):
		...
		receiver_x = []
		message = []
		sender_of_the_message = 'host2' # sender of message = name of the file without '.py'
...
class Level3(Level4):
	def send(self, string):
		self.string = string
		N_byte = 4 # you can change this number
...
class Level1(Level2):
	def send(self, packets):
		...
		os.chdir(main_path + '/host2/output') # change for other host
		print("\nLevel1 fnl:")
		for packet in self.packets:
			name_of_file = str(packet_counter)
			with open(name_of_file, 'w') as file:
				file.write(packet)
			print("Packet[{0}] saved in /host2/output".format(packet_counter)) # change for other host
			packet_counter += 1
	...
	def recv(self):
		...
		for path, dirnames, filenames in os.walk(main_path + '/host2/input'):
			break
